...
## read data in
X_training = pd.read_csv("%s/X_train.csv" % DATA_DIRNAME)
...
#X_training = pd.read_csv("X_proj_train.csv")
#X_validation = pd.read_csv("X_proj_validation.csv")
#X_test = pd.read_csv("X_proj_test.csv")
#
...
if len(unique_labels) <= 2:
  ...
  prediction_fmeasure = metrics.f1_score(y_test, y_pred)
	...
	prediction_precision = metrics.precision_score(y_test, y_pred, average="macro")
	prediction_recall = metrics.recall_score(y_test, y_pred, average="macro")
	prediction_fmeasure = metrics.f1_score(y_test, y_pred, average="macro")
...
performance['algortithm'] = 'Logistic Reg.'
